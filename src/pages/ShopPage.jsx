...
export default function ShopPage() {
  ...
  const { products, loaded, loading, fetchProducts, fetchCategories } =
    useProductsStore();
  ...
  // Refresh product data and categories whenever the shop page is visited
  useEffect(() => {
    fetchProducts();
    fetchCategories();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      {/* Shop Hero */}
      <div className="relative pt-[84px] sm:pt-[92px] pb-14 text-center bg-[var(--bg-card)] border-b border-[var(--border)] overflow-hidden">
        <div
          className="absolute w-[500px] h-[500px] rounded-full pointer-events-none"
          style={{
            background:
              "radial-gradient(circle,rgba(201,168,76,0.4),transparent)",
            filter: "blur(100px)",
            opacity: 0.12,
            top: -200,
            left: "50%",
        ...
        <div className="relative z-10 pt-12">
          <div className="text-[11px] tracking-[0.2em] uppercase text-[var(--gold)] font-semibold mb-4">
            Our Collection
          </div>
          <h1
            className="font-[Playfair_Display] font-black leading-[1.1] mb-4"
            style={{ fontSize: "clamp(44px,6vw,76px)" }}
          >
            Premium Native
            <br />
            <em className="italic font-[400] text-[var(--gold-light)]">
              Fabrics
            </em>
          </h1>
          <p className="text-[var(--text-muted)] text-[17px]">
            From a single yard to bulk rolls — authentic fabrics at honest
            prices.
          </p>
        </div>
      </div>
      ...
      <div className="flex">
        ...
        <main className="flex-1 px-6 lg:px-10 py-8 min-h-[60vh]">
          ...
          {loading && !loaded ? (
            ...
            >
              <p className="font-[Playfair_Display] text-2xl text-[var(--text-ghost)] mb-3">
                No fabrics found
              </p>
